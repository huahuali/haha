...
- (NSData *)getImageSideWithType:(NSInteger)type
{
    NSString *dataStr = @"90";
...
- (NSData *)breakcCheckPoint:(NSData *)totalData
{
    NSString *dataStr = @"8C42455354";
    NSMutableData *fullData = [[NSMutableData alloc] initWithCapacity:1];
    [fullData appendData:[dataStr dataFromHexString_xcy]];
    
    [fullData appendData:[self p_getRandomcode]];
    ...
    [subData appendData:[self p_getRandomcode]];
...
- (NSData *)p_getRandomcode
{
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"randomCodeStr"]) {
        dataStr = [[NSUserDefaults standardUserDefaults] objectForKey:@"randomCodeStr"];
    }
